    ...
    <div className="mb-10 w-full select-none">
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-[var(--accent-glow)] rounded-xl text-[var(--accent-soft)] shadow-glow-accent ring-1 ring-[var(--border)]">
            <Moon className="w-6 h-6 drop-shadow-md" />
          </div>
          <h3 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-[var(--accent-soft)] to-teal-500 bg-clip-text text-transparent tracking-tight">Spánek</h3>
        </div>
        <div className="flex items-baseline gap-1">
          <span className="text-3xl font-black text-[var(--accent-soft)] tabular-nums">{displayValue}</span>
          <span className="text-sm font-medium text-theme-muted">hodin</span>
        </div>
      </div>
      ...
      <div className="relative h-16 group mx-2">
        ...
        <div className="absolute inset-x-2 top-1/2 -translate-y-1/2 h-16 bg-theme-elevated rounded-[2rem] border border-theme-border pointer-events-none transition-colors group-hover:bg-[var(--accent-glow)] shadow-inner" />

        {/* Internal Track Area */}
        <div className="absolute inset-x-6 top-1/2 -translate-y-1/2 h-full pointer-events-none">
          {/* Progress Bar Background */}
          <div className="absolute top-1/2 left-0 right-0 h-3 bg-theme-bg rounded-full -translate-y-1/2 overflow-hidden shadow-inner border border-[var(--border)]">
            {/* Filled Gradient */}
            <motion.div
              className="h-full bg-gradient-to-r from-[var(--accent)] via-[var(--accent-soft)] to-teal-400 will-change-transform"
              style={{ width }}
            />
          </div>
              ...
              <div key={tick} className="flex flex-col items-center gap-2.5">
                <div
                  className={`w-0.5 h-8 -mt-0.5 rounded-full transition-colors duration-100 ${
                    tick <= displayValue ? 'bg-[var(--accent-soft)]/50' : 'bg-theme-border'
                  }`}
                />
                <span
                  className={`absolute top-10 text-xs font-bold transition-colors duration-100 ${
                    tick <= displayValue ? 'text-theme-text' : 'text-theme-muted'
                  }`}
                >
                  {tick}
                </span>
          ...
          {/* Thumb */}
          <motion.div
            className="absolute top-1/2 -ml-3 w-6 h-6 bg-theme-card rounded-full shadow-[0_0_14px_var(--accent-glow)] ring-2 ring-[var(--accent)]/40 z-10 pointer-events-none will-change-transform"
            style={{ left, y: '-50%' }}
          >
            {/* Pulse effect while dragging */}
            <div className={`absolute inset-0 bg-[var(--accent)] rounded-full animate-ping ${isDragging ? 'opacity-30' : 'opacity-0'}`} />
            <div className="absolute inset-0 rounded-full ring-2 ring-[var(--border)] group-hover:ring-[var(--accent)]/30" />
          </motion.div>
        </div>
      </div>
    </div>
